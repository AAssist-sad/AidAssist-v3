/*
  # Schéma de base de données AidAssist
...
-- 3. Aidant roles table (many-to-many between profiles and aided_persons)
...
-- 5. Procedures table (created before appointments to avoid circular dependency)
...
CREATE INDEX IF NOT EXISTS idx_procedures_priority ON procedures(priority);
...
CREATE POLICY "Users can create invitations for their aided persons"
  ...
  WITH CHECK (
    ...
    EXISTS (
      SELECT 1 FROM aidant_roles 
      WHERE aidant_id = auth.uid() 
      AND aided_person_id = invitations.aided_person_id 
      AND role = 'admin' 
      AND status = 'accepted'
    )
  );
...
CREATE TRIGGER update_aided_persons_updated_at 
  BEFORE UPDATE ON aided_persons
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
BEGIN
  -- Get the procedure ID from the trigger
  target_procedure_id := COALESCE(NEW.procedure_id, OLD.procedure_id);
  
  -- Count total and completed steps for the procedure
  SELECT COUNT(*), COUNT(*) FILTER (WHERE completed = true)
  INTO total_steps, completed_steps
  FROM procedure_steps
  WHERE procedure_id = target_procedure_id;
  ...
  -- Update procedure progress and status
  UPDATE procedures 
...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO profiles (id, first_name, last_name, account_type)
  VALUES (
    NEW.id,
    COALESCE(NEW.raw_user_meta_data->>'first_name', 'Utilisateur'),
    COALESCE(NEW.raw_user_meta_data->>'last_name', 'AidAssist'),
    COALESCE(NEW.raw_user_meta_data->>'account_type', 'aidant')::account_type
  );
  RETURN NEW;
END;
